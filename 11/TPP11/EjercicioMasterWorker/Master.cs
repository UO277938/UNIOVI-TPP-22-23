...
namespace EjercicioMasterWorker
{
    ...
    public class Master
    {
        ...
        private List<double> resultado = new List<double>();


        public Master(double[] vector, int numeroHilos)
        {
        ...
        /// </summary>
        public List<double> CalculaValoresSuperioresA(double valorLimite)
        {
            // Creamos los workers
            ...
            for (int i = 0; i < this.numeroHilos; i++)
            {
                ...
                workers[i] = new Worker(this.vector, valorLimite, resultado, indiceDesde, indiceHasta);
            }
            // * Iniciamos los hilos.
